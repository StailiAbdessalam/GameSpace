...
public class Affichage {
    ...
    public void Presantation(Game game, Posts post, Player player, Horaires Time,Queues queueClass){
        ...
        while(true){
            ...
            switch (choixAdmin){
                case 1:
                    ...
                    // choix 2 of Player
                    int IdPostChoix = this.choosePost(postDisponible,player,queueClass);
                    // choix 3 of Player
                    int choixCreneau = this.chooseCreneau(Time,IdPostChoix,player,queueClass);
                    if(choixCreneau==0){
                        continue;
                    }
                    ...
                    //check if post playing
                    boolean IfpostOccuper = this.checkplayer(IdPostChoix,player);
                    if(IfpostOccuper){
                        queueClass.addInQueue(NamePlayer,Game,Timefinish,IdPostChoix);
                    }else {
                ...
                case 2:
                    ...
                    if(player.getAllplayer().size()==0){
                        System.out.println("------------------Vide Player list---------------");
                    }else {
                    ...
                    if(queueClass.getallQueueInList().size()==0){
                        System.out.println("------------------Vide Queues list---------------");
                    }else {
                        System.out.println("-------------Queues list -------------");
                        ArrayList<DetailsPlayer> listPlayer =queueClass.getallQueueInList();
                        listPlayer.forEach((DetailsPlayer Oneplayer)->{
                            System.out.println("Name Player: " +Oneplayer.getNamePlayer()+" || Game: "+Oneplayer.getGame()+ " || Time Finished: "+Oneplayer.timeFinal()+" || in : Post: "+String.valueOf(Oneplayer.getIdPost()+1));
                        });
                    }
    ...
    public int choosePost(ArrayList<DetailsPosts> postDisponible,Player player,Queues queues){
       while (true){
           ...
           for(DetailsPosts post:postDisponible){
               if(allplayer.size()!=0){
                   ...
                   for(DetailsPlayer players:allplayer){
                       String timefinal;
                       if (queues.getQueueByid(post.getId()-1)!=null) {
                           timefinal= String.valueOf(queues.getLastQueueInPostByID(post.getId()));
                           posteDetail+= "\033[0;33m ------playing-----\033[0m"+"end in: \033[0;32m"+timefinal+"\033[0m";
                       }else if(post.getId()==players.getIdPost()+1){
                            timefinal = player.getPlayerByid(players.getIdPost()).timeFinal();
                           posteDetail+= "\033[0;33m ------playing-----\033[0m"+"end in: \033[0;32m"+timefinal+"\033[0m";
                           break;
                       }
                   }
    ...
    public int chooseCreneau(Horaires time,int idpost,Player playerClass,Queues queues){
        while (true) {
            ...
            LocalTime Timeend;
            if(queues.getQueueByid(idpost)!=null){
                Timeend= queues.getLastQueueInPostByID(idpost);
                //Timeend = LocalTime.parse(queues.getQueueByid(idpost).timeFinal());
            }else if(playerClass.getPlayerByid(idpost)!=null){
                 Timeend = LocalTime.parse(playerClass.getPlayerByid(idpost).timeFinal());
            }else{
            ...
            for (DetailsHoraires creneau : allcreneau) {
                ...
                if (nowTime>9 && nowTime<12 && (finishTime>12) || finishTime<9) {
                    System.out.println(i + ": " + creneau.name + "\u001B[31m------invaliable-----\033[0m");
                } else if (((nowTime>=12 && nowTime<14) || (nowTime>14 && (finishTime>=20) ) || finishTime<9) ) {
                    System.out.println(i + ": " + creneau.name + "\u001B[31m------invaliable-----\033[0m");
                } else {
            ...
            if(idcreneauvalid.size()==0){
                try {
                    System.out.println("see you inchaelah");
                    Thread.sleep(2000);
                    return 0;
    ...
    public boolean checkplayer(int idpost,Player playerClass){

        for (DetailsPlayer player:playerClass.getAllplayer()) {
